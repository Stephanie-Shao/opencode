...
2. Run `opencode web` with local UI assets:

```bash
opencode web --port 4096 --hostname 0.0.0.0 --ui-dir /path/to/opencode/packages/app/dist
```

For network access, set auth:

```bash
OPENCODE_SERVER_PASSWORD=secret opencode web --hostname 0.0.0.0 --port 4096 --ui-dir /path/to/opencode/packages/app/dist
```
